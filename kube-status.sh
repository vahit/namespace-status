...
# Create new pane
tmux split -v -p 21 "exec watch --no-title -d kubectl --context ${CONTEXT} --namespace "${NAMESPACE}" get hpa"

tmux select-pane -t 1
tmux split -v -p 47 "exec watch --no-title -d \"kubectl --context ${CONTEXT} --namespace "${NAMESPACE}" top pod | tail --lines +2 | sed 's/  */|/g' | column -s'|' -t\"
 # \"kubectl --context ${CONTEXT} --namespace "${NAMESPACE}" top pod | tail --lines +2 | column\""

# Back to the previuse pane
# tmux select-pane -t 1
tmux split -h -p 40 "exec watch --no-title -d \"kubectl --context ${CONTEXT} top node | tail --lines +2 | sed 's/  */ /g' | cut -f1,3,5 -d ' ' | sed 's/ /\t/g'\""
